...
/datum/status_effect/basic_surgery_state/on_creation(mob/living/new_owner, duration_override, added_state = NONE, removed_state = NONE)
	. = ..()
...
/datum/status_effect/basic_surgery_state/get_examine_text()
	if(HAS_SURGERY_STATE(surgery_state, SURGERY_SKIN_OPEN))
		return "[owner.p_their(TRUE)] skin is open[HAS_SURGERY_STATE(surgery_state, SURGERY_BONE_SAWED) ? " and bones are sawed opened" : ""]."
